...
render(){
    ...
    else
   {
    ...
    useroptions =  useroptions.map((user,index)=>{return {...user, name:`${user.firstname} ${user.lastname}`}})
    useroptions= useroptions.map((user,index)=>(<option 
    key={user.name}>{user.name}</option>))
    if(this.state.isbn==='') {
        this.setState({isbn: defaultBook.isbn,roll: defaultUser.name  });
    }
    return (
  ...
  <h3 align="center">  Issue book</h3>
    ...
    <Form onSubmit={(e) => {
        ...
        let studentid=this.props.users.filter((user)=>(`${user.firstname} ${user.lastname}`===this.state.roll))[0]._id;
        ...
        <FormGroup row>
        <Label htmlFor="roll"> Name  of the student </Label>
        ...
        <FormGroup row>
         ...
         <FormGroup row>
          <Label htmlFor="name_student"> Roll no. of student </Label>
            <Input type="text" id="name_student" name="name_student"
                   placeholder="Name of student" 
                   defaultValue={defaultUser?.roll}
                   value={!this.state.roll?''
                   :this.props.users.filter((user)=>(`${user.firstname} ${user.lastname}`===this.state.roll))[0].roll}
                    className="form-control" disabled/>
         </FormGroup>
         {/* <FormGroup row>
